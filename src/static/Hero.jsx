...
function Hero({ videoData, setVideoData, setFormatLoading, conformDownload }) {
    ...
    const downloadBestVideo = async () => {
        ...
        if (isBlankOrEmpty(videoLink)) {
            toast.error("🔗Oops! No link, no launch! Drop a valid URL to get started. 🚀");
            return;
        }
    ...
    return (
        <div className="flex flex-col items-center  mb-10 mt-22 lg:h-[80vh]  ">
            <div className=" bg-gradient-to-br to-blue-300 to bg-purple-300 w-11/12  flex flex-col justify-center items-center  py-10 px-2 rounded-3xl relative shadow-2xl lg:h-11/12 ">
                ...
                <h1 className=" mb-4 font-extrabold text-4xl md:text-6xl text-center bg-gradient-to-r from-blue-700 via-indigo-400 to-pink-400 bg-clip-text text-transparent">Downloader</h1>
                <p className="text-center text-md mb-6 font-semibold text-sm sm:max-w-2/5">🔥 Unlimited & Hassle-Free! 🚀 Instantly download videos & audio from any platform – fast, easy, and seamless! 🎬🎵</p>
                ...
                <div className="flex flex-row absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-1/2  space-x-4">
                    <Button className="rounded-4xl w-40 mb-2 h-16 bg-blue-700  font-bold hover:bg-indigo-700 " onClick={downloadBestVideo}>
                        <IoMdDownload /> Download Best
                    </Button>

                    {
                        enableFormatBtn && (
                            <Button className="rounded-4xl  w-40 h-15 bg-blue-400 hover:bg-indigo-500 text-black font-bold hover:text-white " onClick={getFormats}>
