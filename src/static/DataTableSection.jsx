...
function DataTableSection({ formats, setFormatId, videoData, formatLoading }) {
  ...
  useEffect(() => {
    if (videoData?.formats) {
      ...
      setVideoFormats(
        videoData?.formats.filter((format) => format.resolution != "audio only")
      );
    }
  }, [videoData]);
    ...
    toast.promise(
      async () => {
        try {
          ...
          const DOWNLOAD_SELECTED_FORMAT_API =
            BASE_URL +
            PATH_SUFIX +
            ...
            "&&videoOnly=" +
            videoOnly;
          const response = await fetch(DOWNLOAD_SELECTED_FORMAT_API, {
            method: "GET",
          });
            ...
            <div className="flex flex-row ">
              ...
              <div className=" flex  items-center mr-2 ">
                <Switch
                  className="mr-1"
                  checked={videoOnly}
                  onCheckedChange={setVideoOnly}
                />
                <Label className={` ${videoOnly ? "font-bold" : ""}`}>
                  Video Only
                </Label>
              </div>
            </div>
                ...
                <TableBody className="font-semibold text-gray-600">
                  {audioFormats.map((format, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        {format.fileSize
                          ? convertSize(format.fileSize)
                          : convertSize(format.fileSizeApprox)}
                      </TableCell>
                      <TableCell>
                        <Button onClick={() => downloadFormat(format.formatId)}>
                          <MdDownloadForOffline />
